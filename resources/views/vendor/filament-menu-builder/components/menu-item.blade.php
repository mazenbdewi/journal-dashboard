...
<li wire:key="{{ $item->getKey() }}" data-sortable-item="{{ $item->getKey() }}"
    ...
    <div
        ...
        <div class="flex flex-1 items-center gap-2 truncate">
            ...
            @if ($hasChildren)
            <x-filament::icon-button icon="heroicon-o-chevron-right" x-on:click="open = !open"
                ...
                color="gray" class="transition duration-200 ease-in-out" x-bind:class="{ 'rotate-90': open }"
                size="sm" />
            @endif
    ...
    <ul x-collapse x-show="open" wire:key="{{ $item->getKey() }}.children"
        x-data="menuBuilder({ parentId: {{ $item->getKey()  }} })" class="mt-2 space-y-2 ms-4">
